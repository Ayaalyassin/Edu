...
class CompleteController extends Controller
{
    ...
    public function get_request_teacher()
    {
        try {
            DB::beginTransaction();
            $date = [];
            $requestCompletes = RequestComplete::with('user')->where('status', 0)->get();
            foreach ($requestCompletes as $requestComplete) {
                if ($requestComplete->user->role_id == 2) {
    ...
    // public function get_request_student()
    // {
    //     try {
    //         DB::beginTransaction();
    //         $date = [];
    //         $requestCompletes = RequestComplete::with('user')->where('status', '=', 0)->get();
    //         foreach ($requestCompletes as $requestComplete) {
    //             if ($requestComplete->user->role_id == 'student') {
    //                 $date[] = $requestComplete;
    //             }
    //         }
    //         DB::commit();
    ...
    public function store(CompleteRequest $request)
    {
        try {
            ...
            $user = auth()->user();
            $re = RequestComplete::where('user_id', $user->id)->first();
            if ($re) {
    ...
    public function destroy($id)
    {
        try {
            DB::beginTransaction();
            $requestComplete = RequestComplete::find($id);
            if (!$requestComplete) {
            ...
            DB::commit();
            return $this->returnData('delete order successfully');
        } catch (\Exception $ex) {
            DB::rollback();
            return $this->returnError($ex->getCode(), $ex->getMessage());
        }
    ...
    public function accept_request_complete_teacher($id)
    {
        try {
            DB::beginTransaction();
            $rate = 0;
            $requestComplete = RequestComplete::with('user')->find($id);
            if (!$requestComplete) {
                return $this->returnError('not found request', 404);
            }
            if ($requestComplete->status == 1) {
